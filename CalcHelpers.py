...
METRICS = {
    ...
    'sharpness': lambda r: _metric(r, lambda x: [
        abs(ev[i] - ev[i - 1]) for ev, *_ in x for i in range(1, len(ev))
    ]),
    'closedness': lambda r: _metric(r, lambda x: [
        pc for _, _, pws, *_ in x for pc in pws
    ]),
    'resign_rate': lambda r: (
        100 * sum(1 for x in lost if x[8]) / len(lost)
        if (lost := [x for x in r if x[7] is False]) else 0
    ),
    'best_move_rate': lambda r: 100 * _metric(r, lambda x: [
...
def pmetrics(r):
    ...
    return (
        {p: sum(d) / len(d) for p, d in acpl.items() if d},
        {p: 100 * c / tot for p, c in cnt.items()}
    )
...
def cmetrics(r):
    ...
    for *_, ct, cs, won, _, _, _, _ in r:
        if cs and won is not None:
            side[cs][won] += 1
            grp = ("<=4" if ct <= 4 else "5-6" if ct <= 6 else "7-8" if ct <= 8 else
                   "9-10" if ct <= 10 else "11-12" if ct <= 12 else "13-15" if ct <= 15 else ">15")
            turn[grp][won] += 1
    return (
        {k: 100 * v[1] / sum(v) if sum(v) else 0 for k, v in side.items()},
        {k: 100 * v[1] / sum(v) if sum(v) else 0 for k, v in turn.items()}
    )
...
def tmetrics(r):
    ...
    for ev, _, _, c, _, _, _, won, _, _, h, _ in r:
        if h is not None:
            if diffs := [max(0, ev[i - 1] - ev[i] if i % 2 else ev[i] - ev[i - 1])
                        for i in range(1, len(ev)) if c is None or i % 2 == c]:
                acpl[h].extend(diffs)
            if won is not None:
                wins[h][won] += 1
    return (
        {h: sum(d) / len(d) for h, d in acpl.items()},
        {h: 100 * v[1] / sum(v) if sum(v) else 0 for h, v in wins.items()}
    )
    ...
    for ev, _, _, c, _, _, _, won, _, _, _, gn in r:
        if gn is not None:
            ...
            if won is not None:
                wins[gn][won] += 1
    return (
        {g: sum(d) / len(d) for g, d in acpl.items()},
        {g: 100 * v[1] / sum(v) if sum(v) else 0 for g, v in wins.items()}
    )
...
def eco_stats(pgns, res, users=None):
    ...
    for pgn, rs in zip(pgns, res):
        ...
        try:
            ...
            ev, *rest = rs
            diffs = [max(0, ev[i - 1] - ev[i] if i % 2 else ev[i] - ev[i - 1])
                    for i in range(1, len(ev)) if c is None or i % 2 == c]
    ...
    return (
        {e: {'a': sum(d['a']) / len(d['a']), 'w': 100 * d['w'] / (d['w'] + d['l']) if d['w'] + d['l'] else 0, 'n': d['w'] + d['l']}
...
def print_stats(lbls, dsets, pgn_sets=None, res_sets=None):
    ...
    print(f"{'-'*w}")
    for p in range(1, 7):
        print(f"{pn[p]+' ACPL':<9} " + ' '.join(f"{x[0].get(p,0):>7.1f}" for x in ps))

    print(f"{'-'*w}")
    for p in range(1, 7):
        print(f"{pn[p]+' %':<9} " + ' '.join(f"{x[1].get(p,0):>7.1f}" for x in ps))
    ...
    print(f"{'-'*w}")
    for side in ["OO", "OOO"]:
        print(f"{side+'-WR':<9} " + ' '.join(f"{x[0].get(side,0):>7.1f}" for x in cs))
    ...
    for i, l in enumerate(lbls):
        if gs[i][0] or gs[i][1]:
            ...
            for g in sorted(set(gs[i][0].keys()) | set(gs[i][1].keys())):
                print(f"  {g:>2} A:{gs[i][0].get(g,0):>5.1f} W:{gs[i][1].get(g,0):>5.1f}%")

    if lbls:
        print(f"\n{lbls[0]} Time of Day:")
        for h in range(24):
            print(f"  {h:02d}h A:{ts[0][0].get(h,0):>5.1f} W:{ts[0][1].get(h,0):>5.1f}%")