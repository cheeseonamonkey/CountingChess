...
def evaluate_single_game(pgn,
                         ...
                         game_num=None):
    try:
        game = chess.pgn.read_game(StringIO(pgn)) if isinstance(pgn,
                                                                str) else pgn
        if not game: return None

        white, black = game.headers.get("White", "").lower(), game.headers.get(
            "Black", "").lower()
        ...
        if result in ["1-0", "0-1"]:
            won = (result == "1-0") == (color == chess.WHITE) if color else (
                result == "1-0")

        hour = None
        if track_time and (d := game.headers.get("UTCDate")) and (
                t := game.headers.get("UTCTime")):
            try:
                hour = datetime.strptime(
                    f"{d} {t}", "%Y.%m.%d %H:%M:%S").replace(
                        tzinfo=ZoneInfo("UTC")).astimezone(
                            ZoneInfo("America/Denver")).hour
            except:
                pass
        ...
        with chess.engine.SimpleEngine.popen_uci(stockfish_path) as engine:
            for move_index, move in enumerate(game.mainline_moves(), 1):
                ...
                evals.append(
                    max(
                        -800,
                        min(800, info["score"].white().score(mate_score=1e4)
                            or 0)))
                piece_types.append(board.piece_type_at(move.from_square))
                pawn_counts.append(
                    sum(
                        len(board.pieces(chess.PAWN, s))
                        for s in [chess.WHITE, chess.BLACK]))

                if not castle_turn and board.is_castling(
                        move) and board.turn == (color or board.turn):
                    castle_turn, castle_side = move_index, "K" if move.to_square > move.from_square else "Q"

                if (not color or move_index % 2 != color) and (pv := info.get(
                        "pv", [None])[0]):
        ...
        return (evals, piece_types, pawn_counts, color,
                int(
                    ...
                    or game.headers.get("BlackElo", 0)), castle_turn,
                castle_side, won, is_resignation, best_moves, hour, game_num)
    except:
        return None
...
def main():
    ...
    user_games, random_games = Fetchers.fetch_all_users_games(
        [user], None)[:20], Fetchers.fetch_random_games(30, 25, 19)
    ...
    bott_games, bott_results = zip(*sorted_pairs[:int(n * 0.2)]) if n else ([],
                                                                            [])
    avg_games, avg_results = zip(
        *sorted_pairs[int(n * 0.1):int(n * 0.9)]) if n else ([], [])
